...
**Raw pp needs GG+. Raw speed does not.** Gate the pp half only, and how you gate it depends on
where the number sits. Two rules:
...
**Leaving raw speed open is a decision, not an oversight.** Only five commands render it as a
standing field, and every other appearance comes through the flag. Gating it would therefore make
the `raw` flag itself a subscriber feature and collapse both rules into one check. Raw speed stays
free instead, so a non-subscriber typing `raw` still gets the board and the graph.
