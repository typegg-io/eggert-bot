...
def log(discord_id, command, server_id="900", user_id=None):
    ...
    command_log.log_command(discord_id, user_id, command, server_id)
...
def test_log_command_reads_the_origin_off_the_server(scratch_db):
    command_log.log_command("1", None, "stats", None)
...
def test_log_command_stringifies_the_discord_id(scratch_db):
    command_log.log_command(123, None, "stats", "900")
...
def test_get_command_usage(scratch_db):
    ...
    assert command_log.get_command_usage("1") == {"stats": 2, "day": 1}
    assert command_log.get_command_usage(1) == {"stats": 2, "day": 1}
    assert command_log.get_command_usage("3") == {}
...
def test_get_all_command_usage(scratch_db):
    ...
    assert command_log.get_all_command_usage() == {"stats": 2, "day": 1}
...
def test_get_command_leaderboard_orders_by_usage(scratch_db):
    ...
    assert command_log.get_command_leaderboard("stats") == [
        {"discord_id": "2", "usages": 2},
        {"discord_id": "1", "usages": 1},
    ]
    assert command_log.get_command_leaderboard("nothing") == []
...
def test_get_top_users_by_command_usage_orders_by_total(scratch_db):
    ...
    assert command_log.get_top_users_by_command_usage() == [
...
def test_get_command_count(scratch_db):
    assert command_log.get_command_count() == 0

    log("1", "stats")
    log("2", "day")

    assert command_log.get_command_count() == 2
...
def test_migrate_command_name_merges_into_the_new_name(scratch_db):
    ...
    assert command_log.migrate_command_name("top", "bestgraph") == 2
    assert command_log.get_all_command_usage() == {"bestgraph": 4, "day": 1}


def test_migrate_command_name_reports_no_users_when_unused(scratch_db):
    log("1", "stats")

    assert command_log.migrate_command_name("nothing", "bestgraph") == 0
    assert command_log.get_all_command_usage() == {"stats": 1}
